...
class Camera(object):
    """
    Base Camera Parameter, including calibration matrix and opencv distortion parameters.
    以此为基类融合单目，双目，RGBD，ToF类型的相机参数
    """
    def __init__(self, fx, fy, cx, cy, invfx, invfy,
                 extrinsic, dist_coef, width, height):
        # 焦距
        self.fx = fx
        self.fy = fy
...
class StereoCamera(Camera):
    ...
    def __init__(self, fx, fy, cx, cy, extrinsic, dist_coef,
                 width, height, baseline, frustum_near, frustum_far):
        super(StereoCamera, self).__init__(fx, fy, cx, cy, extrinsic,
                                           dist_coef, width, height)
        self.frustum_near = frustum_near
        self.frustum_far = frustum_far
        self.baseline = baseline
...
class CalibrationParameter(object):
    def __init__(self, fx, fy, cx, cy, invfx, invfy, mbf, mb, mThDepth):
        self.fx = fx
        self.fy = fy
        self.cx = cx
        self.cy = cy
...
class CameraParameter(Camera):

    def __init__(self, fx, fy, cx, cy, width, height,
                 frustum_near, frustum_far, baseline):
        super(CameraParameter, self).__init__(fx, fy, cx, cy, width, height,
                                              frustum_near, frustum_far, baseline)
